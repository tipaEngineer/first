...
export const App: React.FC<AppProps> = ({products}) => {
    ...
    return (
        <>
            <SearchBar filtering={filtering} setFiltering={setFiltering} isStocked={isStocked} setStocked={setStocked}/>
            <ProductTable products={products} isStocked={isStocked} filtering={filtering}/>
        </>
    )
}
...
const ProductTable: React.FC<ProductTableProps> = ({products, filtering, isStocked}) => {
...
const SearchBar: React.FC<TextProperties> = (props) => {
    ...
    const isStockedHandleOnChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        props.setStocked(!props.isStocked)
    }

    return (
        <>
            <input type={"text"} placeholder={"Search..."} value={props.filtering}
                   onChange={textHandleOnChange}/>
            <p><label><input type={"checkbox"} onChange={isStockedHandleOnChange}/>Only show products in stock</label>
            </p>
        </>
    )
}
